...
function EditArticleContent({ match }) {
  ...
  useEffect(() => {
    if (match.params.id !== "new") {
      ...
      axios({
        method: "get",
        url: `${config.json_server_address}/articles?_sort=id&_order=desc&_limit=1`,
        responseType: "json",
        cancelToken: source.token
      })
        .then((response) => setNewId(response.data[0].id + 1))
        .catch((error) => console.log(error))
  ...
  const handleArticlePost = async (type = null) => {
    try {
      setIsLoading(true)
      const data =
        match.params.id === "new" ? { ...values, id: newId } : { ...values }
      const coverted = await convertImages(data)

      const response = await axios({
        method: "post",
        url: `${config.json_server_address}/add_article`,
        responseType: "json",
        data: {
          ...coverted,
  ...
  const handleArticlePatch = async (type = null) => {
    try {
      ...
      const data = { ...values, isPublished: isPublished }
      const converted = await convertImages(data)

      await axios({
        method: "patch",
        url: `${config.json_server_address}/articles/${values.id}`,
        responseType: "json",
        headers: {
          Authorization: `Bearer ${token}`
        },
              ...
              <div className="data-input__instruments">
                ...
                <div
                  ...
                  onClick={() =>
                    setValues({
                      ...values,
                      content: [...values.content, { type: "quote", value: "" }]
                    })
                  }
                >
                ...
                <div
                  ...
                  onClick={() =>
                    setValues({
                      ...values,
                      content: [...values.content, { type: "image", url: null }]
                    })
                  }
                >
              ...
              <div className="meta-info">
                <p className="publication-time">
                  {values.time === null
                    ...
                    : getPublicationTime(new Date(values.time))}
                </p>
                <ul className="list">
