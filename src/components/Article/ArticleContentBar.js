...
function ArticleContentBar({ article }) {
  ...
  if (article !== null) {
    const { time, tags, title, subtitle, preview, content, author } = article

    return article === null ? null : (
      <motion.section className="content content_overview" variants={animations} initial="hidden" animate="visible" >
        <div className="article-info article-info_overview">
          <div className="meta-info">
            <p className="publication-time">
              {getPublicationTime(new Date(time))}
            </p>
            <ul className="list">
