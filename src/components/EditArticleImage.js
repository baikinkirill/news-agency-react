...
function EditArticleImage({ id, values, setValues }) {
    
    const handleRemove = (id) => {
        setValues({...values, content: values.content.filter((item, index) => index !== id)})
    }
    
    return (
        <div className={(values.content[id].url === null) ? "data-input__container_content-container stripe-background" : "data-input__container_content-container"}>
            <div className="data-input__image" style={(values.content[id].url === null) ? {} : { backgroundImage: `url(${values.content[id].url})` }}>
            {(values.content[id].url !== null) && <div className="darken"></div>}
            <ImageUploading
                value={values.content[id].url}
                onChange={img => setValues({
                    ...values,
            ...
            <div className="data-input__delete-icon" onClick={() => handleRemove(id)}><i className="ri-delete-bin-line"></i></div>
        </div>
    )
}
